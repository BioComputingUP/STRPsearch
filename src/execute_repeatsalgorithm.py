...
def execute_repeatsalgorithm(
    ...
    rprint(f"[bold][{general_utils.get_current_time()}] Trying to find potential hits ...\n")
    ...
    if found_hit:
        ...
        for idx in range(len(target_df)):
            ...
            rprint(
                f"""[bold blue][{general_utils.get_current_time()}] Processing hit {idx + 1}/{len(target_df)} ...
[bold blue][{general_utils.get_current_time()}] Query name: {query_name}
[bold blue][{general_utils.get_current_time()}] Target name: {target_name}
[bold blue][{general_utils.get_current_time()}] Target type: {ct}\n"""
            )
            ...
            if regions_dict:
                if len(regions_dict) > 1:
                    rprint(f"[{general_utils.get_current_time()}] {len(regions_dict)} regions were mapped\n")
                else:
                    rprint(f"[{general_utils.get_current_time()}] 1 region was mapped\n")
                ...
                # Create and save the highlighted PyMOL session
                for region_id, v in regions_dict.items():
                    ...
                    pymol_out_path = os.path.join(temp_query_dir, f"{region_id}_{target_name}_{ct}_{e_val}.pse")

                    general_utils.create_pymol_session(
                        region_id=region_id,
                        structure_path=region_out_path,
                        components=v,
                ...
                rprint(f"[bold yellow]No repeat region was found with the current hit [/bold yellow]\n")
                logging.warning(f"No repeat region was found with the current hit")
                typer.Exit()

        rprint(f"[{general_utils.get_current_time()}] All the hits were processed\n")

        # Transfer the files from temporary directory to the final query output directory
        rprint(f"[{general_utils.get_current_time()}] Transferring files to the output directory ...\n")
        ...
        rprint(f"[{general_utils.get_current_time()}] Job successfully finished\n")
